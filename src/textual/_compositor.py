...
from textual import errors, log
...
class CompositorUpdate:
    ...
    def __init__(self, graphic_regions: list[Region] | None = None) -> None:
        self.graphic_regions = graphic_regions or []
...
class InlineUpdate(CompositorUpdate):
    ...
    def render_segments(self, console: Console) -> str:
        ...
        if len(self.strips) > 1:
            back_lines = len(self.strips) if self.clear else len(
                self.strips) - 1
...
class ChopsUpdate(CompositorUpdate):
    ...
    def __init__(
        ...
        """
        super().__init__(graphic_regions)
        self.chops = chops
        self.spans = spans
        self.chop_ends = chop_ends

    def _intersects_graphics_span(self, y: int, x1: int, x2: int) -> bool:
        for gr in self.graphic_regions:
            if gr.y <= y < gr.bottom:
                if not (x2 <= gr.x or x1 >= gr.right):
                    return True
        return False
...
class Compositor:
    ...
    def __init__(self) -> None:
        ...
        self._graphic_regions: dict[Widget, Region] = {}
        ...
        self._visible_widgets: dict[Widget,
                                    tuple[Region, Region]] | None = None
        ...
        self._layers_visible: list[list[tuple[Widget,
                                              Region, Region]]] | None = None

    def register_graphic_region(self, widget: Widget, region: Region) -> None:
        """Register a persistent graphic region (e.g. SIXEL)."""
        self._graphic_regions[widget] = region

    def unregister_graphic_region(self, widget: Widget) -> None:
        """Unregister a persistent graphic region."""
        self._graphic_regions.pop(widget, None)

    def get_graphic_regions(self) -> list[Region]:
        return list(self._graphic_regions.values())

    def _get_graphics_renders(
        self,
        crop: Region | None = None,
    ):
        from .drivers.graphics import GraphicsRenderable

        for widget, (region, clip) in self.visible_widgets.items():
            if not isinstance(widget, GraphicsRenderable):
                continue

            if widget.preserve_graphics:
                continue

            if crop and not crop.overlaps(clip):
                continue

            render_region = Region.intersection(region, clip)
            if not render_region:
                continue

            yield from widget.render_graphics(render_region)
    ...
    def render_full_update(self, simplify: bool = False) -> LayoutUpdate:
        ...
        graphics = list(self._get_graphics_renders(screen_region))
    ...
    def render_partial_update(self) -> ChopsUpdate | None:
        ...
        if update_regions:
            ...
            crop = Region.from_union(
                update_regions).intersection(screen_region)
        ...
        graphics = list(self._get_graphics_renders(crop))
        update = ChopsUpdate(
            chops,
            spans,
            chop_ends,
            graphic_regions=self.get_graphic_regions(),
        )
        update.graphics = graphics
        return update
    ...
    def _render_chops(
        ...
        for region, clip, strips in renders:
            ...
            for y, strip in zip(render_region.line_range, strips):
                ...
                chops_line = chops[y]
                final_cuts = [cut for cut in cuts[y]
                              if (last_cut >= cut >= first_cut)]
    ...
    def _intersects_graphics(self, region: Region) -> bool:
        for graphic_region in self._graphic_regions.values():
            if graphic_region.intersection(region) is not None:
                return True
        return False

    def _to_viewport_region(self, region: Region) -> Region:
        offset = self._viewport.offset
    ...
    def update_widgets(self, widgets: set[Widget]) -> None:
        ...
        for widget in self.visible_widgets.keys() & widgets:
            ...
            if widget.preserve_graphics:
                log("_compositor: update_widgets Widget ignorado")
                continue
